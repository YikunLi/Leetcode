 ...
 *              of nums2. The Next Greater Number of a number x in nums1 is the
 *              first greater number to its right in nums2. If it does not
...
public class L496NextGreaterElementI {
    ...
    public static class Exhaustion implements Solution {
        ...
        public int[] nextGreaterElement(int[] findNums, int[] nums) {
            ...
            int[] nextGreaterElements = new int[findNums.length];
            for (int i = 0; i < findNums.length; i++) {
                boolean isFindedNumber = false;
                nextGreaterElements[i] = -1;
                for (int j = 0; j < nums.length; j++) {
                    if (isFindedNumber) {
                        if (nums[j] > findNums[i]) {
                            nextGreaterElements[i] = nums[j];
                            break;
                        }
                    } else {
                        isFindedNumber = findNums[i] == nums[j];
                    }
                }
            }
